...
We evaluate SASY policy enforcement against the [tau2-bench](https://github.com/sierra-research/tau2-bench) airline domain.
...
- **Translator model**: Claude Opus 4.7 (passed as
  `translate(..., model="opus")`; the SDK default
  is `"sonnet"` — pass `model="opus"` to reproduce
  these numbers)
