...
result = translate(
    ...
    instructions=(
        ...
        "User messages have msg.agent = \"LLMAgent\" and msg.agent_role = $UserRole().\n"
        "Use custom C++ functors for content checks, not @llm_check_fn."
    ),
...
This writes `output/airline_policy.dl`, `output/airline_functors.cpp`, and `output/agent_summary.md`.
...
| `model` | `haiku` (cheap, weaker), `sonnet` (default), or `opus` |
...
```bash
make upload-policy POLICY=output/airline_policy.dl
```
