...
1. **Stage 1** — an agent reads your codebase and produces `agent_summary.md` describing instrumentation points, tool calls, and message flow.
2. **Stage 2** — a second agent, armed with the summary and the `policy-compiler` skill, writes the Datalog policy + functors.
3. **Server-side validation** — the artifacts are compiled against the toolkit's `common_policy.dl` before being returned.
