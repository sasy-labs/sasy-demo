...
result = translate(
    ...
    instructions="<custom instructions>",  # overrides the analyzer can't infer from code — e.g. "filter messages on msg.agent='LLMAgent'"
)
...
This writes `output/airline_policy.dl` and `output/agent_summary.md`, plus `output/airline_functors.cpp` *only when the policy needs custom C++ helpers* — content matching, date arithmetic, NLP classifiers, etc. The simple Quickstart policy is pure attribute matching, so Soufflé handles every rule natively and no functors file is emitted; the richer [tau2 airline benchmark policy](/benchmarks/tau2-airline/) does produce functors. See `benchmarks/tau2-airline/run.py` for the concrete instructions used to produce the benchmark results.

<Aside>
  Translation takes 5–15 minutes depending on codebase size and model. The SDK polls the job status endpoint and emits progress lines through Python `logging` at INFO under the `sasy` logger; pass `on_progress=lambda stage, elapsed: ...` if you want a programmatic callback (e.g. to drive a progress bar). Configure the endpoint with `SASY_TRANSLATE_URL` (default `https://sasy-translate.fly.dev`).
</Aside>
...
A second translator (`sasy.policy.write_policy`) is available as a prototype for extended policy validation. It emits a truth table and adversarial verifier checks alongside the Datalog; see the [confidence report](/policy/confidence/) for how to read them. `write_policy` is useful for short, self-contained policies but **not recommended** for production agents — it does not read a codebase, so use `translate` for anything non-trivial.
