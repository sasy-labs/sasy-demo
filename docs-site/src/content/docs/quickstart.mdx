...
The §3 example required threading `check_tool_call`
...
- `httpx` and `requests` — outbound HTTP requests are
  recorded into the graph and proxied through SASY
  for credential injection and policy checks.
- Langroid — `ChatAgent` and `Task` messages, tool
  calls, and their dependency edges are recorded;
  tool calls are auto-checked before invocation.
- Tau2-bench agents — user-simulator and assistant
...
same `sasy.instrument()` call covers plain `httpx` /
`requests` and Tau2-bench agents too — no framework
switch needed.
...
# lr.Task drives the agent loop — calling the LLM,
# executing tool calls, and looping. After
# instrument(), every tool call inside the loop is
